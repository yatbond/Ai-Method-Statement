// =============================================================================
// OpenRouter Document AI provider
//
// Sends documents to an OpenRouter model (OpenAI-compatible API) for extraction.
// OpenRouter aggregates many LLM providers behind a single API.
// =============================================================================

import type { ExtractedChunk, DocumentExtractionResult } from "./index";

export class OpenRouterDocumentAIProvider {
  ...
  async extract(buffer: Buffer, mimeType: string): Promise<DocumentExtractionResult> {
    ...
    // For PDF and images, send to OpenRouter via OpenAI-compatible API
    if (mimeType === "application/pdf" || mimeType.startsWith("image/")) {
      return this.extractViaApi(buffer, mimeType);
    }

    throw new Error(`Unsupported MIME type for OpenRouter extraction: ${mimeType}`);
  }
